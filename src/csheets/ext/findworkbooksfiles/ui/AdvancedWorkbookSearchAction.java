...
public class AdvancedWorkbookSearchAction extends BaseAction{
     ...
     * that provides the user a method to choose a diretory on disk. If any files 
     * are found, they're shown through a JTable. The information shown is the file name
     * and his path on disk. If there are no files found, a message will pop up on the screen
     * giving that information to the user.
     * The JTable on which the files are shown, is "clickable". If a user selects a row of the table (file name + path)
     * a JDialog will pop up with and the first line of the workbook will be shown. The user will have have two JButtons
     * on the JDialog: Open and Cancel. The Open button will open the selected workbook. The Cancel button will close the
     * JDialog, returning to the JTable.
     * Because of the fact it will be running through a thread, the JTable information will be added as files are found.
     * 
