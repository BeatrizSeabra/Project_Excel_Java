...
public class ExtensionsManagerWindow extends JFrame {
    ...
    public ExtensionsManagerWindow(UIController uiController) {
        initComponents();
        /**
        ...
        for (UIExtension ex : extensions) {
            if (ex.getMenu() != null) {
                JCheckBox jcb = new JCheckBox(ex.getExtension().getName(), null, ex.getMenu().isEnabled());
                cb.add(jcb);
                jPanel1.add(jcb);              
            }
        }
    ...
    private void applyButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_applyButtonActionPerformed
        ...
        for (JCheckBox c : cb) {
            for (UIExtension ex : extensions) {
                if (ex.getMenu() != null) {
                    if (ex.getExtension().getName().equalsIgnoreCase(c.getText())) {
                        if(c.isSelected() == false){
                            ex.getMenu().setEnabled(false);
                        }else{
                            ex.getMenu().setEnabled(true);
                        }
                    } 
                }
            }
